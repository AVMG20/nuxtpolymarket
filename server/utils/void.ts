...
/**
 * Relic caches a run may bank: one per five minutes of flight, plus the
 * warden's once the run has lasted long enough to have fought one.
 */
export function voidRelicCap(elapsedMs: number, wardenKilled: boolean) {
    return Math.min(4, Math.floor(elapsedMs / 300_000) + (wardenKilled && elapsedMs >= 240_000 ? 1 : 0))
}
