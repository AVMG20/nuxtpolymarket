...
} else if (query.get('town')) {
    const mix: TownBuildingId[] = ['house', 'house', 'farm', 'bakery', 'house', 'mill', 'house', 'smithy', 'emporium']
    for (let t = 0; t < 6; t++) {
        const level = TOWN_VISUAL_LEVELS[Math.min(4, t)]!
        const ox = (t % 3) * 4, oz = Math.floor(t / 3) * 4
        mix.forEach((type, i) => place(createBuildingModel(type, level, t * 3 + i), ox + (i % 3), oz + Math.floor(i / 3)))
    }
} else {
