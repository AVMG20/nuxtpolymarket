// Generates whatever Void Runner sound effects are missing from
// public/void/sound/, via the ElevenLabs text-to-sound-effects API.
//
// The manifest (every event, its prompt, and its trim length) lives in
// app/utils/void-sounds.ts — the single source of truth for both this script
// and in-game playback. Each event gets its own folder with VOID_SOUND_VARIANTS
// numbered takes (<event>/1.wav ..); the script diffs the manifest against
// what's on disk and fills the gaps, so deleting a bad take and re-running
// regenerates just that slot.
//
// Unlike the SHAPEZZ script this one asks the API for raw PCM (not MP3) and
// wraps it in a WAV header itself, so it needs no ffmpeg/afconvert on the box.
// Each clip is then trimmed into a tight one-shot: anchor on the loudest
// transient ('peak') or the first audible sample ('onset'), cut the configured
// length, fade the tail, normalize. The untrimmed clip is kept in a temp folder
// (printed at the end) so you can hand-pick a different take if the auto-cut
// grabbed a bad one.
//
// Run:  bun run scripts/generate-void-sounds.ts [flags]
...
//   --only=<text>   only events whose name includes <text> (e.g. --only=boss)
//
...
import { VOID_SOUND_MANIFEST, VOID_SOUND_VARIANTS, type VoidSoundSpec } from '../app/utils/void-sounds'

/** Raw 16-bit PCM at this rate, mono — no external decoder needed to wrap it. */
const OUTPUT_FORMAT = 'pcm_44100'
const PCM_SAMPLE_RATE = 44100
const ENDPOINT = `https://api.elevenlabs.io/v1/sound-generation?output_format=${OUTPUT_FORMAT}`
...
const OUTPUT_DIR = new URL('../public/void/sound/', import.meta.url)
const RAW_DIR = join(tmpdir(), 'void-sfx-raw')
...
// The API hands back headerless 16-bit signed little-endian PCM (mono at
// PCM_SAMPLE_RATE); wrap it in a RIFF header so the rest of the pipeline can
// treat it exactly like a decoded WAV. Passes any real WAV straight through.
function pcmToWavBytes(bytes: Uint8Array): Uint8Array {
    if (isRiffWav(bytes)) return bytes
    // Drop a trailing odd byte so the 16-bit frames line up.
    ...
    new Uint8Array(buffer, 44).set(bytes.subarray(0, dataSize))
...
function trimToOneShot(wav: WavData, keepSeconds: number, cut: 'peak' | 'onset'): WavData {
    ...
    // void-sounds.ts mean the same thing across clips.
    if (peakValue > 0) {
...
interface ClipJob {
    ...
    spec: VoidSoundSpec
    variant: number
    /** Path relative to OUTPUT_DIR, e.g. "boss-spawn/2.wav". */
    relPath: string
}

async function main() {
    let events = Object.entries(VOID_SOUND_MANIFEST)
    if (only) events = events.filter(([event]) => event.includes(only))

    const jobs: ClipJob[] = events.flatMap(([event, spec]) =>
        Array.from({ length: VOID_SOUND_VARIANTS }, (_, i) => ({
            event,
            spec,
            variant: i + 1,
    ...
    for (const [index, job] of missing.entries()) {
        ...
        try {
            ...
            const decoded = pcmToWavBytes(raw)
