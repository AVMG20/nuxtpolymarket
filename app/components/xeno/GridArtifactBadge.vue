...
import { getArtifact, getEffectValue } from '#shared/utils/xeno'
...
const speedDots = computed(() => {
  ...
  return Math.round(getEffectValue(art.value, 'grid_speed_boost') * 20)
})

const yieldDots = computed(() => {
  if (!art.value) return 0
  return getEffectValue(art.value, 'grid_yield_bonus')
})
</script>

<template>
  <div v-if="slot?.artifact" class="absolute top-1.5 left-1.5 flex flex-col items-start gap-1 z-10">
    <div class="flex items-center gap-0.5 bg-muted rounded-full px-1.5 py-0.5 mb-1">
