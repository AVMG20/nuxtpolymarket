...
export class MeadowbrawlGame {
    ...
    get damageMult(): number {
        return (1 + 0.15 * this.stack('might')) * (1 + 0.12 * this.stack('oversized'))
    }

    get attackSpeed(): number {
        return 1 + 0.12 * this.stack('haste')
    }

    get moveSpeed(): number {
        return 1 + 0.1 * this.stack('swift')
    }
    ...
    private updatePlayer(dt: number, first: boolean) {
        ...
        if (!p.attack || p.attack.phase === 'recovery') p.buffer = Math.max(0, p.buffer - dt)
        ...
        if (p.dodge) {
            ...
            // Rooted during slams and sweeps.
    ...
    private advanceSwing(a: PlayerAttack) {
        ...
        if (a.phase === 'recovery' && a.t >= a.recovery) {
            ...
            p.comboTimer = finished ? 0 : this.weapon.comboWindow
            if (finished) p.comboHits = 0
        }
    }
    ...
    private playerHitEnemy(e: Enemy, def: SwingDef) {
        const heavy = !!def.finisher || this.weapon.heavy
        const dmg = this.weapon.baseDamage * this.damageMult * def.damage
        const dealt = this.damageEnemy(e, dmg, {
            source: this.player, heavy, knockback: def.knockback, stagger: def.stagger, tag: 'melee'
        })
        if (dealt > 0) {
            const p = this.player
            p.comboHits += 1
    ...
    damageEnemy(e: Enemy, amount: number, opts: {
        ...
        knockback?: number
        stagger?: number
        tag: 'melee' | 'special' | 'proj' | 'sprint' | 'whirl' | 'shock' | 'lightning' | 'burn' | 'explode'
        ...
        const direct = opts.tag === 'melee' || opts.tag === 'special' || opts.tag === 'proj' || opts.tag === 'sprint' || opts.tag === 'whirl'
        ...
        const color = opts.color ?? (opts.tag === 'burn' ? '#ff9a3c' : opts.tag === 'lightning' ? '#c9a3ff' : opts.tag === 'explode' ? '#ffb347' : heavy ? '#ffe066' : '#ffffff')
        const size = opts.tag === 'burn' ? 11 : heavy ? 20 : 14 + Math.min(6, dmg / 20)
        this.floaters.push({ x: e.x + (Math.random() - 0.5) * 14, y: e.y, z: e.def.height + 4, text: String(dmg), life: 0.8, maxLife: 0.8, color, size, vx: (Math.random() - 0.5) * 30 })
        ...
        if (direct) {
            this.burst(e.x, e.y, e.def.height * 0.5, heavy ? 12 : 6, 'blood', '#a3121f', heavy ? 160 : 110, 0.6, kbDir)
            this.burst(e.x, e.y, e.def.height * 0.5, heavy ? 8 : 4, 'spark', '#fff2c4', 200, 0.25, kbDir)
            this.hitstop = Math.max(this.hitstop, (0.03 + Math.min(0.08, dmg / 320)) * (heavy ? 1.4 : 1))
            this.shake = Math.max(this.shake, 1.5 + Math.min(10, dmg / 12) + (heavy ? 3 : 0))
            this.emit(heavy ? 'heavyHit' : 'hit', e.x, e.y, Math.min(1, dmg / 60))
            this.onHitProcs(e, dmg, opts.tag)
    ...
    hurtPlayer(amount: number, from: Vec, knockback: number): boolean {
        const p = this.player
        ...
        p.comboTimer = 0
        p.comboHits = 0
        if (p.special?.kind !== 'dash') p.special = null
        p.dodge = null
                ...
                case 'attack': {
                    ...
                    if (a.kind === 'charge') {
                        ...
                        if (!a.hit && inSegment({ x: ox, y: oy }, e, e.r + 2, p, p.r)) {
                            a.hit = true
                            if (this.hurtPlayer(a.damage, { x: e.x - Math.cos(a.dir) * 10, y: e.y - Math.sin(a.dir) * 10 }, a.knockback)) {
    ...
    private releaseAttack(e: Enemy) {
        ...
        switch (a.kind) {
            case 'melee': {
                ...
                if (inArc(e, a.dir, a.halfAngle, a.reach, p, p.r)) this.hurtPlayer(a.damage, e, a.knockback)
                e.state = 'recover'
                e.stateT = 0
                break
            }
            case 'slam': {
                ...
                if (inCircle(e, a.radius, p, p.r)) this.hurtPlayer(a.damage, e, a.knockback)
                e.state = 'recover'
                e.stateT = 0
                break
            }
            ...
            if (pr.owner === 'enemy') {
                ...
                for (const e of this.enemies) {
                    ...
                    if (Math.hypot(pr.x - e.x, pr.y - e.y) <= pr.r + e.r) {
                        pr.hitIds.add(e.id)
                        this.damageEnemy(e, pr.damage, { source: { x: pr.x - pr.vx * 0.01, y: pr.y - pr.vy * 0.01 }, knockback: 90, stagger: 0.15, tag: 'proj' })
